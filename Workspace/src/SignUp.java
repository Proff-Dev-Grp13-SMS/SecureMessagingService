...
public class SignUp implements ActionListener {		//public class that implements actionlistener, which are just actions that are performed when buttons are pressed for example
	...
	public SignUp() 
	{
		...
		JTextField Name = new JTextField ("Name"); //adds a text field for login 
		...
		button = new JButton("Sign Up");		//New button "sign up", will need position moved
	        button2 = new JButton("Back to Login"); 		//New button "sign up", will need position moved
...
//		LoginText.addActionListener(this);
		...
		Login.setBounds(50,50,710,1270); //sets the position and the layout of the text field 
                Login.setBounds(70,70,760,1290);  
		...
		panel.add(Name); //adds the panel 
		panel.add(Login); //this adds the 2nd text field underneath the login button but we need to change the layout of the buttons 
		panel.add(Password);
		panel.add(button);	
		panel.add(button2);				// ^^^^^^^^^^^^^
	...
	public static void main(String[] args) {			//main method
		new SignUp();						//creates new frame and reloads everything in LoginGUI
	...
	public void actionPerformed(ActionEvent e) {			//action events when buttons are pressed, this was a pain in the ass to do
		...
		if (e.getActionCommand()=="Back to Login") //if the button 'Login' is pressed
        {	
